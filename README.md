...
## Настройка
1. Открыть веб-интерфейс и начать создание кластера Kubernetes выбранной версии.
2. В случае возникновения ошибки на этапе создания кластера в веб-интерфейсе, открыть консоль разработчика нажатием F12 и приступить к анализу ошибок.
3. Проверить квоты проекта, если ошибка в консоли указывает на их недостаток, и при необходимости увеличить квоты или удалить ненужные объекты в проекте.
4. Убедиться в создании объекта magnum-кластера и heat-кластера после начала процесса создания кластера.
5. Проверить создание объектов OpenStack для будущего кластера, включая сети, порты, loadbalancer, диски и инстансы.
6. В случае обнаружения ошибок на этапах создания объектов OpenStack, использовать инструменты Heat для их диагностики и устранения.

...
1. Проверьте квоты проекта. Если ошибка в консоли указывает на их недостаток, то при необходимости увеличьте квоты или удалите ненужные объекты в проекте.
...
1. Проверьте, что создались объекты magnum-кластера и heat-кластера после начала создания кластера.
...
3. Для диагностики и устранения ошибок на этапах создания объектов OpenStack используйте инструменты Heat или дополнительные средства.
### Как изменить квоты?
...
1. Просмотрите детали существующей квоты по команде:
   
 ```bash
   kubectl describe resourcequota <quota-name> -n <namespace_test>
   ```

1. Откройте конфигурацию квоты по команде:
```bash
   kubectl edit resourcequota <quota-name> -n <namespace_test>
```

1. Измените параметры квоты.
...
| **Хранилище данных**  | Общий объем временного хранения | `requests.ephemeral-storage`
|  | Лимит временного хранения для всех подов | `limits.ephemeral-storage`
| **Счётчики объектов** | Максимальное количество подов | `pods`
|  | Количество сервисов | `services`
|  | Максимальное количество сервисов с типом LoadBalancer | `services.loadbalancers`
|  | Максимальное количество секретных переменных | `secrets`
